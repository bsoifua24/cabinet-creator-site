...
- "type" is whichever of these four the space is CLOSEST to for layout purposes — pick the nearest fit, don't invent new types:
  kitchen (sink+range/fridge run, or a wet bar/butler's pantry/prep kitchen/prep pantry/scullery showing actual counter or cabinet runs — a double-ruled rectangle along a wall, with or without a sink),
  bath (vanity/sink + fixtures, incl. powder rooms),
  laundry (washer/dryer/utility sink),
  closet (storage-only space with no counter and no plumbing fixture — a plain shelving pantry, mudroom lockers, linen, office built-ins, media walls, garage storage). A room labeled "pantry" or "prep" is NOT automatically closet — check whether it actually shows counter/cabinet symbols before deciding.
